...
@use_as: python3 CNN.py mix/path/mix.wav sp1/path/sp1.wav sp2/path/sp2.wav {"train", "test"}

Train and/or Test of the CNN model.
"""
...
# Clip some time instances, mandatory for proper reshape later
mix = mix[:-57]
sp1 = sp1[:-57]
sp2 = sp2[:-57]
...
# CNN Model Train. Model Saved upon completion.
if mode == "train":

    # reshape arrays into 10-instances time sequences
    ...
    model.fit([X_train, X_train], [Y_train, Y_train2], validation_data=([X_eval, X_eval], [Y_eval, Y_eval2]), epochs=ep,
              batch_size=b, callbacks=[es])
    model.save('CNN.hdf5')
    ring_a_bell()

# CNN Model Test. Output is the reconstructed .wav files for each speaker. Evaluation follows.
if mode == "test":
    ...
    out_path = "Results/CNN/" + str(ts) + "__" + str(ack) + "/"
    if not os.path.exists(out_path):
        os.makedirs(out_path)

    X_test = array_reshape(X_test, 10) # reshape array into 10-instances time sequence

    model = tf.keras.models.load_model('CNN.hdf5', custom_objects={'LeakyReLU': tf.keras.layers.LeakyReLU},
                                       compile=False)
